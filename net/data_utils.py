...
class RESIDE_Dataset(data.Dataset):
    def __init__(self,path,train,size=crop_size,format='.png'):#################################################
        ...
        image_type = self.haze_imgs_dir[0].split('.')[-1]
        print('image_type=', image_type)
        self.format=format #.png
        self.format='.'+image_type 
...
pwd=os.getcwd()
print(pwd)
path='/home/graduate/weihao/FFA-Net/data'#path to your 'data' folder

drug_train_loader=DataLoader(dataset=RESIDE_Dataset(path+'/white_data/train',train=True,size=crop_size),batch_size=BS,shuffle=True)
drug_test_loader=DataLoader(dataset=RESIDE_Dataset(path+'/white_data/val',train=False,size='whole img'),batch_size=1,shuffle=False)
